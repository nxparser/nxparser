 ...
 */
package nxparser.org.apache.commons.rdf.api;
...
public enum RDFSyntax {
	 ...
	 */
	public static RDFSyntax byMediaType(String mediaType) {
		final String type = mediaType.toLowerCase(Locale.ENGLISH).
				split("\\s*[;,]", 2)[0];
		for (RDFSyntax rdfsynt : RDFSyntax.values()) {
			if (rdfsynt.mediaType.equals(type))
				return rdfsynt;
		}	
		return null;
	}
	 ...
	 *         extension was found.
	 */
	public static RDFSyntax byFileExtension(String fileExtension) {		
		final String ext = fileExtension.toLowerCase(Locale.ENGLISH);
		for (RDFSyntax rdfsynt : RDFSyntax.values()) {
			if (rdfsynt.fileExtension.equals(ext))
				return rdfsynt;
		}	
		return null;	
	}
